      ...
      {tab==="custom"&&(<section style={S.card}>
        {!customPlan?(<div style={{textAlign:"center",padding:"20px 0"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            ...
            <button onClick={deleteCustomPlan} style={S.linkDanger}>Plan löschen</button>
          </div>
          ...
          {customPlan.days.some((d)=>d.main.length||d.warmup.length||d.cooldown.length)&&(<div style={{marginTop:20}}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:8}}>Tracking</div>
            ...
            {customPlan.days.map((d)=>(<div key={d.id} style={S.cpDay}>
              ...
              {[...d.warmup,...d.main,...d.cooldown].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);const k=`${d.id}:${entry.id}:${cWeek}`;const log=customLog[k]||{};return (
                <div key={idx} style={S.cpRow}>
                  <div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:12.5}}>{ex?ex.name:entry.id}</div><div style={{fontSize:10,color:"#8A7E70"}}>Ziel: {entry.sets}×{entry.reps}</div></div>
                  <div style={{display:"flex",alignItems:"center",gap:4}}>
                    <input type="number" placeholder="kg" value={log.w??""} onChange={(e)=>logCustom(d.id,entry.id,"w",e.target.value)} style={S.cpNum}/>
                    <span style={S.cpX}>kg ·</span>
