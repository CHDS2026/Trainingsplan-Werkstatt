...
function build(t) {
  ...
  s += (t.extra || "");
  s += '<line x1="8" y1="168" x2="122" y2="168" stroke="#C8BBAA" stroke-width="2" stroke-dasharray="5 4"/>';
  for (const [j1, j2, w] of t.bones) {
    const a1 = A[j1], a2 = A[j2], b1 = B[j1], b2 = B[j2];
    s += `<line stroke="#1A1410" stroke-width="${w}" stroke-linecap="round" x1="${a1[0]}" y1="${a1[1]}" x2="${a2[0]}" y2="${a2[1]}">`
       + an("x1", a1[0], b1[0]) + an("y1", a1[1], b1[1]) + an("x2", a2[0], b2[0]) + an("y2", a2[1], b2[1]) + "</line>";
  }
  const ha = A.head, hb = B.head;
  s += `<circle fill="#1A1410" cx="${ha[0]}" cy="${ha[1]}" r="11">` + an("cx", ha[0], hb[0]) + an("cy", ha[1], hb[1]) + "</circle>";
  if (t.bar) {
    const [j, half] = t.bar, a = A[j], b = B[j];
    s += `<line stroke="#C2410C" stroke-width="6" stroke-linecap="round" x1="${a[0]-half}" y1="${a[1]}" x2="${a[0]+half}" y2="${a[1]}">`
       + an("x1", a[0]-half, b[0]-half) + an("y1", a[1], b[1]) + an("x2", a[0]+half, b[0]+half) + an("y2", a[1], b[1]) + "</line>";
    for (const sg of [-1, 1])
      s += `<circle fill="#C2410C" cx="${a[0]+sg*half}" cy="${a[1]}" r="6">` + an("cx", a[0]+sg*half, b[0]+sg*half) + an("cy", a[1], b[1]) + "</circle>";
  }
  return s + "</svg>";
}
...
const T = {
  ...
  bench: { A:{head:[34,103],sho:[48,103],hip:[92,103],knee:[104,130],ankle:[104,160],hand:[60,72]},
           ...
           extra:'<rect x="26" y="108" width="84" height="12" rx="3" fill="#EADFD4" stroke="#C8BBAA" stroke-width="2"/><line x1="40" y1="120" x2="40" y2="160" stroke="#C8BBAA" stroke-width="4"/><line x1="96" y1="120" x2="96" y2="160" stroke="#C8BBAA" stroke-width="4"/>' },
  ...
  hipthrust: { A:{head:[28,92],sho:[44,96],hip:[80,128],knee:[100,122],ankle:[106,158]},
           ...
           extra:'<rect x="18" y="98" width="34" height="10" rx="3" fill="#EADFD4" stroke="#C8BBAA" stroke-width="2"/>' },
  legext: { A:{head:[40,58],sho:[46,70],hip:[62,104],knee:[94,104],ankle:[98,140]},
           ...
           extra:'<rect x="40" y="104" width="34" height="10" rx="3" fill="#EADFD4" stroke="#C8BBAA" stroke-width="2"/><line x1="46" y1="114" x2="46" y2="158" stroke="#C8BBAA" stroke-width="4"/>' },
  ...
  dips:  { A:{head:[60,36],sho:[60,52],hip:[62,92],knee:[64,118],ankle:[72,148],hand:[46,58]},
           ...
           extra:'<line x1="40" y1="58" x2="40" y2="120" stroke="#C8BBAA" stroke-width="3"/>' },
};
...
const MAP = {
  ...
  bench_lh:"bench", bench_kh:"bench", incline_lh:"bench", incline_kh:"bench", incline_m:"bench", chestpress_m:"bench", chestpress_c:"bench",
