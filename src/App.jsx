...
export default function App(){
  ...
  const moveCustomEx=(dayId,section,from,to)=>setCustomPlan((c)=>({...c,days:c.days.map((d)=>{
  ...
  const nextSession=useMemo(()=>{
    ...
    if(customPlan&&customPlan.days&&customPlan.days.length){
      const withWd=customPlan.days.map((d,i)=>{const hit=WD_FULL.findIndex((w)=>d.name.toLowerCase().includes(w.toLowerCase()));return {d,i,wd:hit};});
      ...
      const first=customPlan.days.find((d)=>d.main.length>0);
...
{customActive?(<>
          ...
          {(()=>{const charts=[];customPlan.days.forEach((d)=>{[...d.main,...d.warmup,...d.cooldown].forEach((entry)=>{const ser=customSeries(d.id,entry.id,entry);if(ser.length>=2){const ex=EXERCISES.find((e)=>e.id===entry.id);charts.push({key:d.id+entry.id,name:ex?ex.name:entry.id,day:d.name,ser});}});});
