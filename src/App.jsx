...
export default function App(){
  ...
  const nextSession=useMemo(()=>{
    // 1) Eigener Plan hat Vorrang
    const curDays=daysForWeek(customPlan,cWeek);
    if(curDays&&curDays.length){
      const withWd=curDays.map((d,i)=>{const hit=WD_FULL.findIndex((w)=>d.name.toLowerCase().includes(w.toLowerCase()));return {d,i,wd:hit};});
      const dated=withWd.filter((x)=>x.wd>=0&&x.d.main.length>0);
      if(dated.length){
        for(let off=0;off<7;off++){const want=(todayIdx+off)%7;const hit=dated.find((x)=>x.wd===want);
          if(hit)return {title:hit.d.name,items:hit.d.main,warm:hit.d.warmup,cool:hit.d.cooldown,when:off===0?"Heute":off===1?"Morgen":WD_FULL[want],source:"custom",dayId:hit.d.id};}
      }
      const first=curDays.find((d)=>d.main.length>0);
      if(first)return {title:first.name,items:first.main,warm:first.warmup,cool:first.cooldown,when:"Als Nächstes",source:"custom",dayId:first.id};
    }
    ...
    return null;
  },[customPlan,schedule,todayIdx,cWeek]);
    ...
    <div style={S.page}>
      ...
      {nextSession&&(<section style={S.nextCard}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:8}}>
          ...
          {nextSession.source==="auto"&&nextSession.time&&<span style={{fontSize:11,color:"#8A7E70"}}>~{nextSession.time} min</span>}
        </div>
        ...
        <div style={{marginTop:8}}>
          {nextSession.items.slice(0,6).map((it,i)=>{const ex=EXERCISES.find((e)=>e.id===it.id);const nm=ex?ex.name:(it.name||it.id);
            ...
            return (<div key={i} style={S.nextRow}><span style={{fontSize:12.5,fontWeight:600}}>{nm}</span><span style={{fontSize:11.5,color:"#8A7E70"}}>{detail}</span></div>);})}
          {nextSession.items.length>6&&<div style={{fontSize:11,color:"#8A7E70",marginTop:4}}>… und {nextSession.items.length-6} weitere</div>}
        </div>
        <button onClick={()=>setTab(nextSession.source==="custom"?"track":"plan")} style={S.nextBtn}>{nextSession.source==="custom"?"Training starten & tracken":"Zum vollständigen Plan"}</button>
      </section>)}
...
{customActive?(<>
          ...
          {daysForWeek(customPlan,cWeek).map((d)=>(<div key={d.id} style={S.cpDay}>
...
const BLUE="#2E6FE0", BLUE_T="#6FA3FF";      // 20 kg – primär
...
input[type=range]{accent-color:${BLUE_T}}
...
function chip(a){return {padding:"8px 14px",borderRadius:999,fontSize:13,fontWeight:700,cursor:"pointer",border:`1px solid ${a?BLUE:LINE}`,background:a?BLUE:SUNK,color:a?"#fff":DUST};}
...
function poolRow(on){return {display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 12px",borderRadius:10,cursor:"pointer",border:`1px solid ${on?BLUE:LINE}`,background:on?"#17253A":RACK,color:CHALK};}
