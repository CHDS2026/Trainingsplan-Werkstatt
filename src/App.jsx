        ...
        {!customPlan?(<div style={{textAlign:"center",padding:"20px 0"}}>
          ...
          <div style={S.revBox}>
            ...
            {(customPlan.revs||[]).length>0&&(<div style={{marginTop:8}}>
              <div style={{fontSize:11,color:"#8A7E70",marginBottom:4}}>Bestehende Planänderungen:</div>
            ...
            {[["warmup","Aufwärmen","#B45309"],["main","Hauptübungen","#8B0A0A"],["cooldown","Abwärmen","#0F766E"]].map(([sec,label,col])=>(
              <div key={sec} style={{marginBottom:10}}>
                ...
                {d[sec].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);
                  ...
                  return (
                  <div key={idx} ref={(el)=>{rowRefs.current[`${d.id}:${sec}:${idx}`]=el;}} style={{padding:"8px 0",borderBottom:`1px solid ${LINE}`,opacity:isDragging?0.4:1,background:isTarget?"#FBEDE4":"transparent",borderTop:isTarget?`2px solid ${RUST}`:"2px solid transparent",transition:"background .12s"}}>
          ...
          {daysForWeek(customPlan,cWeek).filter((d)=>!focusDay||d.id===focusDay).map((d)=>{const dDone=isDone(d.id,cWeek);const hasWork=d.main.length>0;return (<div key={d.id} style={{...S.cpDay,opacity:dDone?0.62:1,borderLeft:dDone?`3px solid ${GREEN}`:"3px solid transparent"}}>
            ...
            {["warmup","main","cooldown"].flatMap((sec)=>d[sec].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);const k=`${d.id}:${entry.id}:${cWeek}`;const log=customLog[k]||{};const ser=customSeries(d.id,entry.id,entry);
              ...
              <div key={sec+idx} style={{borderBottom:`1px solid ${LINE}`,padding:"10px 0"}}>
                ...
                <div style={{fontSize:10,color:"#8A7E70",marginBottom:6}}>Ziel {entry.sets}×{entry.reps}{ex?" · "+ex.muscles.join(", "):""}</div>
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:11,color:"#5C5247"}}>Gewicht</span>
                  <input type="number" value={wShown} onChange={(e)=>logW(d.id,entry.id,e.target.value)} style={S.cpNum}/><span style={S.cpX}>kg</span>
                  {prevAllDone&&!stored&&<span style={{fontSize:10,color:"#0F766E",fontWeight:700}}>↑ +2,5 kg (Vorwoche geschafft)</span>}
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:5}}>
                  {Array.from({length:cnt}).map((_,i)=>{const st=(Array.isArray(log.sets)&&log.sets[i])||{};return (
                    <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:11,color:"#8A7E70",width:46}}>Satz {i+1}</span>
                      ...
                      <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",marginLeft:"auto"}}><input type="checkbox" checked={!!st.done} onChange={(e)=>logSet(d.id,entry.id,i,"done",e.target.checked)} style={{width:19,height:19,accentColor:RUST}}/><span style={{fontSize:11,fontWeight:600,color:st.done?"#15803D":"#A8998A"}}>{st.done?"✓":"offen"}</span></label>
                    </div>);})}
                </div>
                {allDoneNow&&<div style={{fontSize:10.5,color:"#15803D",fontWeight:700,marginTop:6}}>✓ Alle Sätze geschafft – nächste Woche automatisch +2,5 kg</div>}
                {ser.length>1&&(<div style={{height:110,marginTop:8}}><ResponsiveContainer width="100%" height="100%"><LineChart data={ser} margin={{top:6,right:8,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="#EADFD4"/><XAxis dataKey="week" tick={{fontSize:10,fill:"#8A7E70"}} tickFormatter={(w)=>`W${w}`}/><YAxis yAxisId="kg" tick={{fontSize:10,fill:"#8A7E70"}} domain={["dataMin - 5","dataMax + 5"]}/><YAxis yAxisId="reps" orientation="right" tick={{fontSize:10,fill:"#B45309"}} allowDecimals={false} domain={[0,"dataMax + 2"]}/><Tooltip formatter={(v,n)=>n==="reps"?[`${v} Wdh`,"Wdh gesamt"]:[`${v} kg`,n==="e1rm"?"e1RM":"Gewicht"]} labelFormatter={(w)=>`Woche ${w}`}/><Line yAxisId="kg" type="monotone" dataKey="weight" stroke={RUST} strokeWidth={2.5} dot={{r:3}}/><Line yAxisId="kg" type="monotone" dataKey="e1rm" stroke="#0F766E" strokeWidth={2} strokeDasharray="4 3" dot={{r:2}}/><Line yAxisId="reps" type="monotone" dataKey="reps" stroke="#B45309" strokeWidth={2} dot={{r:2}}/></LineChart></ResponsiveContainer></div>)}
              </div>);}))}
