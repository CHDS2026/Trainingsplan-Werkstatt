...
export default function App(){
  ...
  const cpDefaults={warmup:{sets:2,reps:10},main:{sets:4,reps:8},cooldown:{sets:1,reps:30}};
  ...
  const addCustomEx=(dayId,section,exId)=>{const def=cpDefaults[section];setCustomPlan((c)=>({...c,days:c.days.map((d)=>d.id===dayId?{...d,[section]:[...d[section],{id:exId,sets:def.sets,reps:def.reps}]}:d)}));setAdding(null);setExQuery("");};
  ...
  const buildExamplePlan=()=>{
    const wu=()=>[
      {id:"Seilspringen + Foam Rolling (5–10 Min)",sets:1,reps:1},
      {id:"Scapular Push-ups",sets:2,reps:15},
      {id:"Scapular Pull-ups",sets:2,reps:15},
      {id:"Scapular Dips",sets:2,reps:12},
    ];
    return {weeks:8,days:[
      {id:uid(),name:"Samstag · Bench & Planche",warmup:wu(),main:[
        {id:"planche",sets:5,reps:10},{id:"bench_lh",sets:4,reps:5},{id:"row_lh",sets:4,reps:8},
        {id:"latraise_c",sets:3,reps:12},{id:"facepull",sets:3,reps:18}],cooldown:[]},
      {id:uid(),name:"Sonntag · Squat & Recovery",warmup:wu(),main:[
        {id:"squat_lh",sets:4,reps:5}],cooldown:[{id:"jogging",sets:1,reps:1}]},
      {id:uid(),name:"Montag · Calisthenics",warmup:wu(),main:[
        {id:"muscleup",sets:5,reps:5},{id:"oap",sets:3,reps:4},{id:"dips",sets:4,reps:10},
        {id:"lsit",sets:3,reps:12}],cooldown:[]},
      {id:uid(),name:"Dienstag · Pull & Cardio",warmup:wu(),main:[
        {id:"handstand",sets:1,reps:10},{id:"pulldown_w",sets:4,reps:10},{id:"row_c",sets:3,reps:11},
        {id:"curl_kh",sets:3,reps:12}],cooldown:[{id:"bike_erg",sets:1,reps:1}]},
      {id:uid(),name:"Mittwoch · Deadlift & Levers",warmup:wu(),main:[
        {id:"frontlever",sets:5,reps:8},{id:"deadlift",sets:3,reps:3},{id:"weightedpull",sets:4,reps:7},
        {id:"calf_st",sets:3,reps:15}],cooldown:[]},
      {id:uid(),name:"Donnerstag · Push / OHP",warmup:wu(),main:[
        {id:"ohp_lh",sets:4,reps:5},{id:"cgbench",sets:4,reps:8},{id:"dips",sets:4,reps:8},
        {id:"triceps_c",sets:3,reps:12}],cooldown:[]},
    ...
    schedule.forEach((sd)=>{ if(!sd.trainings||!sd.trainings.length)return;
      sd.trainings.forEach((tr)=>{ const day={id:uid(),name:`${sd.day} · ${(tr.title||"Training").split(":")[0]}`,warmup:[],main:[],cooldown:[]};
        tr.blocks.forEach((b)=>{ b.items.forEach((it)=>{
          if(b.tag==="Warm-up") day.warmup.push({id:it.n,sets:1,reps:1});
          else if(b.tag==="Cool-down") day.cooldown.push({id:it.n,sets:1,reps:1});
          else {const sr=parseSR(it.s); day.main.push({id:it.id,sets:sr.sets,reps:sr.reps});}
        });});
        days.push(day);
      });
    });
...
{customActive?(<>
          ...
          {customPlan.days.map((d)=>(<div key={d.id} style={S.cpDay}>
            ...
            {[...d.warmup,...d.main,...d.cooldown].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);const k=`${d.id}:${entry.id}:${cWeek}`;const log=customLog[k]||{};const ser=customSeries(d.id,entry.id);return (
              <div key={idx} style={{borderBottom:`1px solid ${LINE}`,padding:"8px 0"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                  <div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:12.5}}>{ex?ex.name:entry.id}</div><div style={{fontSize:10,color:"#8A7E70"}}>Ziel {entry.sets}×{entry.reps}{ex?" · "+ex.muscles.join(", "):""}</div></div>
                  <div style={{display:"flex",alignItems:"center",gap:4}}>
                    <input type="number" placeholder="kg" value={log.w??""} onChange={(e)=>logCustom(d.id,entry.id,"w",e.target.value)} style={S.cpNum}/><span style={S.cpX}>kg</span>
