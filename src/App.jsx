...
function weeklySchedule(plan, daysPerWeek){
  if(plan.length===0) return WEEKDAYS.map((d)=>({day:d,trainings:[]}));
  const slots=Math.min(Math.max(1,daysPerWeek),7);
  const used=Math.min(slots,Math.max(plan.length<=slots?plan.length:slots,1));
  const pat=DAYPAT[used]||[0,1,2,3,4,5,6].slice(0,used);
  const buckets=Array.from({length:used},()=>[]);
  plan.forEach((w,i)=>buckets[i%used].push(w)); // mehr Einheiten als Tage -> kombiniert
...
export default function App(){
  ...
  const [picked,setPicked]=useState(()=>autoSelect(blendFocus([{key:'hypertrophy',weight:3},{key:'strength',weight:2}]),Object.fromEntries(ALL_EQUIP.map((k)=>[k,true])),{}, 'advanced'));
  ...
  useEffect(()=>{ (async()=>{ const d=await loadState(); if(d){ try{
    ...
    if(d.oneRM)setOneRM(d.oneRM); if(d.picked)setPicked(d.picked); if(d.equip)setEquip(d.equip);
  ...
  const schedule=useMemo(()=>weeklySchedule(plan,daysPerWeek),[plan,daysPerWeek]);
  ...
  const recCond=useMemo(()=>recommend(blend,"conditioning",equip,injuries,4),[blend,equip,injuries]);
    ...
    <div style={S.page}>
      ...
      {tab==="build"&&(<>
        <section style={S.card}>
          ...
          <button onClick={()=>setPicked(autoSelect(blend,equip,injuries,expL))} style={S.recBtn}>✨ Musterplan für diesen Fokus erzeugen</button>
          <div style={S.hint}>Die App wählt automatisch die effektivsten Übungen passend zum Volumen-Budget. Du kannst danach alles manuell anpassen.</div>
        </section>
        ...
        {blend.condW>=0.4&&(<section style={S.card}>
        ...
        <section style={S.card}>
          {pickedMains.length>0&&(<>
            <div style={S.cardLabel}>07 · Kraftwerte (1RM) — nur Grundübungen</div>
        ...
        <section style={S.card}>
          <button onClick={()=>setShowAll((v)=>!v)} style={S.toggleAll}>{showAll?"▾":"▸"} 08 · Alle {EXERCISES.length} Übungen frei wählen</button>
        ...
        <section style={{...S.card,position:"sticky",bottom:12,boxShadow:"0 -4px 24px rgba(0,0,0,.12)"}}>
