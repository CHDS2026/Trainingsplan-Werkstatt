...
export default function App(){
  ...
  useEffect(()=>{ (async()=>{ const d=await loadState(); if(d){ try{
    ...
    if(d.injuries)setInjuries(d.injuries); if(d.track)setTrack(d.track); if(d.history)setHistory(d.history); if(d.ratings)setRatings(d.ratings); if(d.weightLog)setWeightLog(d.weightLog); if(d.customPlan)setCustomPlan(d.customPlan); if(d.customLog)setCustomLog(d.customLog); if(d.doneLog)setDoneLog(d.doneLog);
  }catch(e){} } setHydrated(true); })(); },[]);
  // bei jeder Änderung speichern (erst nach dem Laden)
  useEffect(()=>{ if(!hydrated)return; saveState({focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog,customPlan,customLog,doneLog}); },
    [hydrated,focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog,customPlan,customLog,doneLog]);
  ...
  const resetFocus=()=>{ if(!confirm("Geladenen Plan entfernen und wieder selbst konfigurieren? Der Plan und dein Tracking-Fortschritt werden gelöscht."))return;
    setCustomPlan(null); setCustomLog({}); setDoneLog({}); setFocusDay(null); setUnlocked(null); setMasterCode(""); setTab("build");
  ...
  const buildMasterPlan=()=>{
    ...
    return {source:"dane",weeks:8,days:[
      ...
      {id:"dane-b",name:"Tag B · Squat & Legs",warmup:wu(),main:[
        ...
        {id:"hacksquat",sets:3,reps:11,kg:80},
...
{customActive?(<>
          ...
          {daysForWeek(customPlan,cWeek).filter((d)=>!focusDay||d.id===focusDay).map((d)=>{const dDone=isDone(d.id,cWeek);const hasWork=d.main.length>0;return (<div key={d.id} style={{...S.cpDay,opacity:dDone?0.62:1,borderLeft:dDone?`3px solid ${GREEN}`:"3px solid transparent"}}>
            ...
            {["warmup","main","cooldown"].flatMap((sec)=>d[sec].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);const k=`${d.id}:${entry.id}:${cWeek}`;const log=customLog[k]||{};const ser=customSeries(d.id,entry.id,entry);
              ...
              if(sec==="main"&&isSkill(entry.id)){const st0=(Array.isArray(log.sets)&&log.sets[0])||{};return (
                <div key={sec+idx} style={{borderBottom:`1px solid ${LINE}`,padding:"10px 0"}}>
              ...
              return (
              <div key={sec+idx} style={{borderBottom:`1px solid ${LINE}`,padding:"10px 0"}}>
                <div {...longPress({dayId:d.id,section:sec,idx,entry,week:cWeek})} style={{fontWeight:600,fontSize:12.5,...S.pressable}}>{ex?ex.name:entry.id}</div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:6,flexWrap:"wrap"}}>
                  ...
                  <input type="number" value={wShown} onChange={(e)=>logW(d.id,entry.id,e.target.value)} style={S.cpNum}/><span style={S.cpX}>kg</span>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5}}>
                  {Array.from({length:cnt}).map((_,i)=>{const st=(Array.isArray(log.sets)&&log.sets[i])||{};return (
                    <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:11,color:"#98A2AE",width:46}}>Satz {i+1}</span>
                      <input type="number" placeholder={String(entry.reps)} value={st.reps??""} onChange={(e)=>logSet(d.id,entry.id,i,"reps",e.target.value)} style={S.cpNum}/><span style={S.cpX}>Wdh</span>
                      <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",marginLeft:"auto"}}><input type="checkbox" checked={!!st.done} onChange={(e)=>logSet(d.id,entry.id,i,"done",e.target.checked)} style={{width:19,height:19,accentColor:RUST}}/><span style={{fontSize:11,fontWeight:600,color:st.done?"#45C98D":"#8A94A1"}}>{st.done?"✓":"offen"}</span></label>
                    </div>);})}
                </div>
