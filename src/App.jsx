...
export default function App(){
  ...
  const HOLD_IDS=["wallsit","deadhang","plank","hollowhold"];
...
{customActive?(<>
      ...
      {exMenu&&(<div style={S.overlay} onClick={()=>{setExMenu(null);setExQuery("");}}>
        <div style={S.sheet} onClick={(e)=>e.stopPropagation()}>
          {(()=>{const ex=EXERCISES.find((e)=>e.id===exMenu.entry.id);const nm=ex?ex.name:exMenu.entry.id;
            ...
            if(exMenu.step==="pick")return (<>
              <div style={S.sheetTitle}>Ersetzen durch</div>
              <input autoFocus placeholder="Übung suchen…" value={exQuery} onChange={(e)=>setExQuery(e.target.value)} style={S.cpSearch}/>
              <div style={{maxHeight:260,overflowY:"auto",marginTop:8}}>
                {EXERCISES.filter((e)=>e.id!==exMenu.entry.id&&(e.name.toLowerCase().includes(exQuery.toLowerCase())||e.muscles.some((mm)=>mm.toLowerCase().includes(exQuery.toLowerCase())))).slice(0,40).map((e)=>(
                  <button key={e.id} onClick={()=>setExMenu({...exMenu,pick:e.id,step:exMenu.section==="main"?"carry":"scope",carry:true})} style={S.cpPick}>
                    <span style={{fontWeight:600,fontSize:12.5}}>{e.name}</span>
            ...
            return (<>
              ...
              <div style={S.sheetSub}>{exMenu.mode==="remove"?`${nm} entfernen`:`${nm} ersetzen`}</div>
              ...
              <button onClick={()=>setExMenu({...exMenu,step:exMenu.mode==="remove"?"action":"carry"})} style={S.sheetCancel}>Zurück</button>
            </>);
          })()}
        </div>
      </div>)}
