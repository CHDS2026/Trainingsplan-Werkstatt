...
function buildPlan(sel, blend, expKey, budget, phaseIdx, oneRM, trainDays){
  ...
  templates.forEach((pats)=>{
    const grp=mainsByEff.filter((m)=>pats.includes(m.pattern));
    const primary=grp[0];
    if(!primary) return;
    const rest=grp.slice(1).filter((m)=>moveKey(m)!==moveKey(primary));
    const secondary=rest.find((m)=>m.muscles[0]!==primary.muscles[0]) || rest[0]; // bevorzugt andere Muskelgruppe (z.B. Kniebeuge + Kreuzheben statt 2× Quad)
    const dayMains=secondary?[primary,secondary]:[primary];
...
function autoSelect(blend, equip, injuries, expKey){
  ...
  // 1a) Grundmuster IMMER abdecken: Knie, Hüfte (Kreuzheben!), Druck, Zug — das effektivste je Muster
  ["squat","hinge","push","pull"].forEach((pat)=>{ const b=mains.find(({e})=>e.pattern===pat); if(b&&!pick[b.e.id]){pick[b.e.id]="main";patSeen.add(pat);addVol(b.e.muscles,sets);} });
  // 1b) weitere Grundübungen anderer Muster nach Effektivität, bis Obergrenze
  const cap=Math.min(6,Math.max(wantMains,4));
  mains.forEach(({e})=>{ if(Object.values(pick).filter((c)=>c==="main").length>=cap)return; if(pick[e.id])return; if(patSeen.has(e.pattern))return; pick[e.id]="main"; patSeen.add(e.pattern); addVol(e.muscles,sets); });
...
function sessionTemplates(mainDays){
  ...
  switch(mainDays){
    ...
    case 5: return [["push"], ["pull"], ["squat","olympic"], ["hinge"], U]; // Squat- & Hinge-Tag getrennt
...
export default function App(){
  ...
  useEffect(()=>{ (async()=>{ const d=await loadState(); if(d){ try{
    ...
    if(d.injuries)setInjuries(d.injuries); if(d.track)setTrack(d.track); if(d.history)setHistory(d.history); if(d.ratings)setRatings(d.ratings); if(d.weightLog)setWeightLog(d.weightLog);
  }catch(e){} } setHydrated(true); })(); },[]);
  // bei jeder Änderung speichern (erst nach dem Laden)
  useEffect(()=>{ if(!hydrated)return; saveState({focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog}); },
    [hydrated,focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog]);
