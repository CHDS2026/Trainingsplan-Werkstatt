...
export default function App(){
  ...
  const longPress=(payload)=>({
    onPointerDown:()=>{clearTimeout(pressT.current);pressT.current=setTimeout(()=>setExMenu({...payload,step:"action"}),520);},
  ...
  const applyExChange=(scope)=>{ const m=exMenu; if(!m)return;
    ...
    (scope==="only"?mutateOnly:mutateFrom)(cWeek,fn);
    setExMenu(null); setExQuery("");
    setMsg(scope==="only"?`Nur Woche ${cWeek} geändert`:`Geändert ab Woche ${cWeek}`); setTimeout(()=>setMsg(""),1800);
  };
...
{customActive?(<>
          ...
          {daysForWeek(customPlan,cWeek).filter((d)=>!focusDay||d.id===focusDay).map((d)=>{const dDone=isDone(d.id,cWeek);const hasWork=d.main.length>0;return (<div key={d.id} style={{...S.cpDay,opacity:dDone?0.62:1,borderLeft:dDone?`3px solid ${GREEN}`:"3px solid transparent"}}>
            ...
            {["warmup","main","cooldown"].flatMap((sec)=>d[sec].map((entry,idx)=>{const ex=EXERCISES.find((e)=>e.id===entry.id);const k=`${d.id}:${entry.id}:${cWeek}`;const log=customLog[k]||{};const ser=customSeries(d.id,entry.id,entry);
              if(sec!=="main")return (<div key={sec+idx} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${LINE}`}}><div {...longPress({dayId:d.id,section:sec,idx,entry})} style={{fontSize:12.5,color:"#AEB8C4",userSelect:"none",WebkitUserSelect:"none",cursor:"pointer"}}>{ex?ex.name:entry.id}</div><div style={{fontSize:12,fontWeight:700,color:"#98A2AE"}}>{entry.min??5} Min</div></div>);
              ...
              <div key={sec+idx} style={{borderBottom:`1px solid ${LINE}`,padding:"10px 0"}}>
                <div {...longPress({dayId:d.id,section:sec,idx,entry})} style={{fontWeight:600,fontSize:12.5,userSelect:"none",WebkitUserSelect:"none",cursor:"pointer"}}>{ex?ex.name:entry.id}</div>
