...
export default function App(){
  ...
  const nextSession=useMemo(()=>{
    // 1) Eigener Plan hat Vorrang – offene Einheit der aktiven Woche
    const open=sessionsOf(activeWeek).filter((d)=>!isDone(d.id,activeWeek));
    if(open.length){
      const withWd=open.map((d)=>({d,wd:WD_FULL.findIndex((w)=>d.name.toLowerCase().includes(w.toLowerCase()))}));
      const dated=withWd.filter((x)=>x.wd>=0);
      if(dated.length){
        for(let off=0;off<7;off++){const want=(todayIdx+off)%7;const hit=dated.find((x)=>x.wd===want);
          if(hit)return {title:hit.d.name,items:hit.d.main,warm:hit.d.warmup,cool:hit.d.cooldown,when:off===0?"Heute":off===1?"Morgen":WD_FULL[want],source:"custom",dayId:hit.d.id,week:activeWeek};}
        const firstDated=dated.slice().sort((a,b)=>a.wd-b.wd)[0];
        return {title:firstDated.d.name,items:firstDated.d.main,when:WD_FULL[firstDated.wd],source:"custom",dayId:firstDated.d.id,week:activeWeek};
      }
      const f=open[0];
      return {title:f.name,items:f.main,warm:f.warmup,cool:f.cooldown,when:"Als Nächstes",source:"custom",dayId:f.id,week:activeWeek};
    }
  ...
  const startSession=()=>{ if(nextSession&&nextSession.week)setCWeek(nextSession.week); setTab("track"); if(typeof window!=="undefined")window.scrollTo({top:0,behavior:"smooth"}); };
...
{customActive?(<>
          ...
          {daysForWeek(customPlan,cWeek).map((d)=>{const dDone=isDone(d.id,cWeek);const hasWork=d.main.length>0;return (<div key={d.id} style={{...S.cpDay,opacity:dDone?0.62:1,borderLeft:dDone?`3px solid ${GREEN}`:"3px solid transparent"}}>
            ...
            {hasWork&&<button onClick={()=>toggleDone(d.id,cWeek)} style={dDone?S.dayUndoBtn:S.dayDoneBtn}>{dDone?"Abhaken rückgängig":"✓ Training abhaken"}</button>}
          </div>);})}
