...
export default function App(){
  ...
  const [focusMode,setFocusMode]=useState(null);   // "dane"|"gulnaz" – fokussierter Plan-Modus
  useEffect(()=>{ if(focusMode&&(tab==="build"||tab==="plan"||tab==="home"))setTab("track"); },[focusMode,tab]);
  ...
  useEffect(()=>{ (async()=>{ const d=await loadState(); if(d){ try{
    ...
    if(d.injuries)setInjuries(d.injuries); if(d.track)setTrack(d.track); if(d.history)setHistory(d.history); if(d.ratings)setRatings(d.ratings); if(d.weightLog)setWeightLog(d.weightLog); if(d.customPlan)setCustomPlan(d.customPlan); if(d.customLog)setCustomLog(d.customLog); if(d.doneLog)setDoneLog(d.doneLog); if(d.focusMode)setFocusMode(d.focusMode);
  }catch(e){} } setHydrated(true); })(); },[]);
  // bei jeder Änderung speichern (erst nach dem Laden)
  useEffect(()=>{ if(!hydrated)return; saveState({focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog,customPlan,customLog,doneLog,focusMode}); },
    [hydrated,focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings,weightLog,customPlan,customLog,doneLog,focusMode]);
  ...
  const resetFocus=()=>{ if(!confirm("Geladenen Plan entfernen und wieder selbst konfigurieren? Der Plan und dein Tracking-Fortschritt werden gelöscht."))return;
    setFocusMode(null); setCustomPlan(null); setCustomLog({}); setDoneLog({}); setFocusDay(null); setUnlocked(null); setMasterCode(""); setTab("build");
  ...
  const buildMasterPlan=()=>{
    ...
    return {weeks:8,days:[
      {id:uid(),name:"Tag C · OHP & OAPU",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag F · Pull & Muscle-Ups",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag G · Ruhetag (Rest Day)",warmup:[],main:[],cooldown:[]},
      {id:uid(),name:"Tag A · Bench & Planche",warmup:wu(),main:[
        {id:"planche",sets:5,reps:10,kg:0},
        {id:"bench_lh",sets:5,reps:5,kg:117.5},
        {id:"row_lh",sets:4,reps:8,kg:80},
        {id:"latraise_c",sets:3,reps:15,kg:12},
        {id:"facepull",sets:3,reps:15,kg:20}],cooldown:cd()},
      {id:uid(),name:"Tag B · Squat & Legs",warmup:wu(),main:[
  ...
  const loadMaster=()=>{ if(customPlan&&!confirm("Master-Plan laden? Dein aktueller eigener Plan wird ersetzt."))return;
    setCustomPlan(buildMasterPlan()); setCustomLog({}); setDoneLog({}); setFocusDay(null); setEditWeek(1); setCWeek(1); setFocusMode("dane"); setTab("track");
  ...
  const buildGulnazPlan=()=>{
    ...
    return {weeks:8,days:[
      {id:uid(),name:"Tag A · Rücken, Bizeps & Zugkraft",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag B · Cardio & Regeneration",warmup:[],main:[],cooldown:[
        {id:"jogging",min:30},{id:"Statisches Dehnen (sanft)",min:10}]},
      {id:uid(),name:"Tag C · Brust, Schultern & Trizeps",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag D · Beine, Po & Core",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag E · Ruhetag (Rest Day)",warmup:[],main:[],cooldown:[]},
      {id:uid(),name:"Tag F · Ganzkörper & Metabolik-Volumen",warmup:wu(),main:[
      ...
      {id:uid(),name:"Tag G · Ausdauer & Fettverbrennung",warmup:[],main:[],cooldown:[
        {id:"jogging",min:45}]},
    ]};
  };
  const loadGulnaz=()=>{ if(customPlan&&!confirm("Plan Gülnaz laden? Dein aktueller eigener Plan wird ersetzt."))return;
    setCustomPlan(buildGulnazPlan()); setCustomLog({}); setDoneLog({}); setFocusDay(null); setEditWeek(1); setCWeek(1); setFocusMode("gulnaz"); setTab("track");
  ...
  return (
    <div style={S.page}>
      ...
      {nextSession&&(<section style={S.nextCard}>
        {nextSession.allDone?(<>
      ...
      <nav style={S.tabs}>{(focusMode?[["custom","Eigener Plan"],["track","Tracking"],["eval","Auswertung"],["nutri","Ernährung"],["data","Sicherung"]]:[["build","Aufbauen"],["plan","Plan"],["custom","Eigener Plan"],["track","Tracking"],["eval","Auswertung"],["nutri","Ernährung"],["data","Sicherung"]]).map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={tabBtn(tab===k)}>{l}</button>)}</nav>
