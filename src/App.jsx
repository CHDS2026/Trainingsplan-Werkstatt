...
function buildPlan(sel, blend, expKey, budget, phaseIdx, oneRM, trainDays){
  ...
  templates.forEach((pats)=>{
    const dayMains=mainsByEff.filter((m)=>pats.includes(m.pattern)).slice(0,2); // max 2 Grundübungen/Tag
    if(dayMains.length===0) return;
    ...
    const kraftItems=dayMains.map((m)=>{ addVol(m.muscles,sets); m.muscles.forEach((mm)=>dayMuscles.add(mm)); usedIds.add(m.id); used+=TIME_COST.main;
      return {id:m.id,n:m.name,s:`${sets}×${reps}\u2013${repHi}`,w:oneRM[m.id]?`${(Math.round((oneRM[m.id]*pct)/2.5)*2.5).toFixed(1)} kg`:`${Math.round(pct*100)}% 1RM`,track:true}; });
    ...
    const dA=assist.filter(fam).filter((e)=>!usedIds.has(e.id)).map((e)=>({e,s:blendEff(e,blend)})).sort((a,b)=>b.s-a.s).map((x)=>x.e);
...
function weeklySchedule(plan, daysPerWeek, budget){
  ...
  const ordered=reorderForRecovery(plan);
...
function sessionTemplates(mainDays){
  ...
  switch(mainDays){
    ...
    case 2: return [U, L];
    case 3: return [["push"], ["pull"], ["squat","hinge","olympic"]];
    case 4: return [U, L, U, L];
    case 5: return [["push"], ["pull"], ["squat","hinge","olympic"], U, L];
    default: return [["push"], ["pull"], ["squat","hinge","olympic"], ["push"], ["pull"], ["squat","hinge","olympic"]];
  }
}
...
export default function App(){
  ...
  useEffect(()=>{ if(!hydrated)return; saveState({focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings}); },
    [hydrated,focuses,planWeeks,daysPerWeek,recovery,expL,budget,week,bw,height,bodyfat,activity,oneRM,picked,equip,injuries,track,history,ratings]);
    ...
    <div style={S.page}>
      ...
      {tab==="build"&&(<>
        ...
        {planMode==="manual"&&(<><section style={S.card}>
          ...
          <div style={S.exGrid}>{EXERCISES.filter((e)=>e.cat==="main").map((m)=>{const on=!!picked[m.id];const blocked=m.muscles.some((mm)=>injuries[mm])||!(m.equip.some((q)=>equip[q])||m.equip.includes("kg"));const rec=recMains.find((r)=>r.e.id===m.id);return <button key={m.id} disabled={blocked} onClick={()=>toggle(m.id)} style={exCard(on,blocked)}><div style={{display:"flex",justifyContent:"space-between",gap:6}}><span style={{fontWeight:700,fontSize:12.5}}>{rec?"★ ":""}{m.name}</span><span style={{display:"flex",alignItems:"center",gap:6}}><InfoButton id={m.id} open={infoFor===m.id} onToggle={()=>setInfoFor(infoFor===m.id?null:m.id)}/><span>{blocked?"🚫":on?"●":"○"}</span></span></div><div style={S.exMeta}>{m.muscles.join(" · ")} · {Math.round(blendEff(m,blend))}/10</div>{infoFor===m.id&&EXERCISE_INFO[m.id]&&<div style={S.infoBox}>{EXERCISE_INFO[m.id].t}{EXERCISE_INFO[m.id].c&&<div style={S.infoWarn}>⚠ Komplexe Technik – am besten mit Anleitung/Video lernen.</div>}</div>}</button>;})}</div>
        ...
        <section style={S.card}>
          ...
          <div style={S.exGrid}>{recCond.map(({e,score})=>{const blocked=!(e.equip.some((q)=>equip[q])||e.equip.includes("kg"));return <button key={e.id} disabled={blocked} onClick={()=>toggle(e.id)} style={exCard(!!picked[e.id],blocked)}><div style={{display:"flex",justifyContent:"space-between",gap:6}}><span style={{fontWeight:700,fontSize:12.5}}>★ {e.name}</span><span style={{display:"flex",alignItems:"center",gap:6}}><InfoButton id={e.id} open={infoFor===e.id} onToggle={()=>setInfoFor(infoFor===e.id?null:e.id)}/><span>{blocked?"🚫":picked[e.id]?"●":"○"}</span></span></div><div style={S.exMeta}>{e.muscles.join(" · ")} · {Math.round(score)}/10</div>{infoFor===e.id&&EXERCISE_INFO[e.id]&&<div style={S.infoBox}>{EXERCISE_INFO[e.id].t}{EXERCISE_INFO[e.id].c&&<div style={S.infoWarn}>⚠ Komplexe Technik – am besten mit Anleitung/Video lernen.</div>}</div>}</button>;})}</div>
        </section>
        ...
        <section style={S.card}>
          ...
          {showAll&&["assist","isolation","skill","conditioning"].map((cat)=>(<div key={cat} style={{marginTop:12}}><div style={S.catHead}>{CAT_LABELS[cat]}</div><div style={S.exGrid}>{EXERCISES.filter((e)=>e.cat===cat).map((e)=>{const blocked=e.muscles.some((m)=>injuries[m])||!(e.equip.some((q)=>equip[q])||e.equip.includes("kg"));return <button key={e.id} disabled={blocked} onClick={()=>toggle(e.id)} style={exCard(!!picked[e.id],blocked)}><div style={{display:"flex",justifyContent:"space-between",gap:6}}><span style={{fontWeight:700,fontSize:12}}>{e.name}</span><span style={{display:"flex",alignItems:"center",gap:6}}><InfoButton id={e.id} open={infoFor===e.id} onToggle={()=>setInfoFor(infoFor===e.id?null:e.id)}/><span>{blocked?"🚫":picked[e.id]?"●":"○"}</span></span></div><div style={S.exMeta}>{e.muscles.join(" · ")} · {Math.round(blendEff(e,blend))}/10</div>{infoFor===e.id&&EXERCISE_INFO[e.id]&&<div style={S.infoBox}>{EXERCISE_INFO[e.id].t}{EXERCISE_INFO[e.id].c&&<div style={S.infoWarn}>⚠ Komplexe Technik – am besten mit Anleitung/Video lernen.</div>}</div>}</button>;})}</div></div>))}
        </section></>)}
